...
class VoiceControlAssistant:
    def __init__(self):
        ...
        self._spell_allowed = set([
            ...
            'comillas','comillassimples','más','menos','asterisco','dolar','abrirparentesis','cerrarparentesis','abrirllave','cerrarllave',
            ...
            if config_vals:
                ...
                if vosk_lang:
                    ...
                    if re.fullmatch(r"[a-z]{2}(-[A-Z]{2})?", v) or re.fullmatch(r"[a-z]{2}", v, re.I):
                        self.language = v
                # allow config to override spelling prefix word (e.g. 'tecla')
                spell_pref = config_vals.get('spell_prefix') or config_vals.get('spell-prefix')
                if spell_pref:
                    try:
                        self.spell_prefix = spell_pref.strip()
                    except Exception:
                        pass
                # allow config to override command prefix word (e.g. 'comando')
                command_pref = config_vals.get('command_prefix') or config_vals.get('command-prefix')
                if command_pref:
                    try:
                        self.command_prefix = command_pref.strip()
                    except Exception:
                        pass
    ...
    def _init_command_handlers(self):
        ...
        for k, ch in spell_map.items():
            self._command_handlers[k] = (lambda m, t, n, ch=ch: (pyautogui.typewrite(ch) or self.notify(f'Tecla {ch} enviada') or True))
        ...
        for k, ch in special_map.items():
            self._command_handlers[k] = (lambda m, t, n, ch=ch: (pyautogui.typewrite(ch) or self.notify(f"Símbolo '{ch}' enviado") or True))

        # Ensure simple regex patterns exist for special symbol keys so they are recognized
        try:
            for k in list(special_map.keys()):
                # Normalize common variants (remove accents/extra spaces) and register pattern
                key_normalized = k.replace(' ', '').replace('á', 'a').replace('é', 'e').replace('í', 'i').replace('ó', 'o').replace('ú', 'u')
                if k not in self.commands:
                    self.commands[k] = re.compile(rf"\b{re.escape(k)}\b", re.I)
                if key_normalized not in self.commands:
                    self.commands[key_normalized] = re.compile(rf"\b{re.escape(key_normalized)}\b", re.I)
        except Exception:
            pass

        # puntoyaparte = '.' + Enter
        self._command_handlers['puntoyaparte'] = lambda m, t, n: (pyautogui.typewrite('.') or pyautogui.press('enter') or self.notify('Punto y aparte enviado') or True)

        # Accent dead-keys: press the accent and wait for the next letter (no extra state required)
        self._command_handlers['acento'] = lambda m, t, n: (pyautogui.typewrite("'") or self.notify('Acento agudo enviado') or True)
        self._command_handlers['grave'] = lambda m, t, n: (pyautogui.typewrite('`') or self.notify('Acento grave enviado') or True)
        self._command_handlers['circunflejo'] = lambda m, t, n: (pyautogui.typewrite('^') or self.notify('Circunflejo enviado') or True)
        self._command_handlers['dieresis'] = lambda m, t, n: (pyautogui.typewrite('"') or self.notify('Diéresis enviada') or True)
    ...
    def _strip_command_prefix(self, text: str):
        ...
        try:
            ...
            p = re.escape(prefix)
            return re.sub(rf"^\s*{p}\b[\s,:-]*", "", text, flags=re.I)
        except Exception:
            return text
    ...
    def _show_recognition_system_message(self, text: str):
        """Show a system notification with the first 50 chars and the command info."""
        try:
            display = (text or "")[:50]
    ...
    def _listen_loop(self):
        ...
        with mic as source:
            # Calibrate and tune recognizer to reduce loss of the first word.
            # Use a slightly longer ambient calibration and a lower fixed
            # energy threshold so quiet initial phonemes are not lost.
            try:
                self.recognizer.adjust_for_ambient_noise(source, duration=1.5)
            except Exception as exc:
                print("No se pudo ajustar ruido ambiente:", exc)
            # Use dynamic thresholding but keep a conservative base value
            try:
                self.recognizer.dynamic_energy_threshold = True
                # lower the initial energy threshold to be more sensitive
                self.recognizer.energy_threshold = 150
            except Exception:
                pass
            # Tweak timing parameters to avoid clipping the start of speech
            # and to be tolerant of short pauses inside a phrase.
            ...
            while self.running:
                ...
                if not self.active:
                    ...
                    continue
                try:
                    # Allow a slightly longer timeout waiting for phrase start
                    # to avoid truncating the first phoneme when the user
                    # begins speaking immediately after activation.
                ...
                if not text:
                    continue
                # Mostrar primeros 50 caracteres y el comando reconocido (o 'no reconocido')
                self._show_recognition_system_message(text)
