...
DEFAULT_PATTERNS = {
    ...
    "open_app": r"\b(abrir|iniciar|inicia|executar|ejecutar|abrir programa|abrir aplicación|abrir aplicação)\b",
    ...
    "menu": r"\b(?:opci[oó]n\s+menu)\s+(.+)",
    "new_pattern": r"\b(nuevo patrón)\b",  # Example of a new pattern to be added
}
...
class VoiceControlAssistant:
    ...
    def _handle_command(self, text: str):
        ...
        # Menu pattern: try configured pattern first, fallback to built-in
        menu_pat = self.commands.get("menu")
        if menu_pat:
            mm = menu_pat.search(text)
        else:
        ...
        # Button pattern: try configured pattern first, fallback to built-in
        button_pat = self.commands.get("button")
        if button_pat:
            m = button_pat.search(text)
        else:
