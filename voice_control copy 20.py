    ...
    def _run_overlay(self):
        ...
        def _poll_hide_request():
            if self._hide_requested.is_set():
                ...
                return
            try:
                root.after(50, _poll_hide_request)
            except Exception:
                pass
...
class VoiceControlAssistant:
    ...
    def _click_grid_cell(self, cell_number):
        ...
        if self.grid.visible:
            # Hide the overlay first so the click reaches the underlying window.
            self.grid.hide()
            time.sleep(0.12)
    ...
    def _handle_cell_click(self, match, text, normalized):
        try:
            ...
            max_cell = self.grid.rows * self.grid.cols
            if cell < 1 or cell > max_cell:
                self.notify(f"Celda fuera de rango (1-{max_cell})")
                return True
