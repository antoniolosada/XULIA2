...
class VoiceControlAssistant:
    def __init__(self):
        ...
        self._correction_cmd_queue = Queue()
    ...
    def _dictate_text(self, phrase):
        try:
            ...
            try:
                # try to read the window title if hwnd is available
                if hwnd:
                    try:
                ...
                # fallback to raw text if something fails
    ...
    def _correct_last_phrase(self):
        ...
        if not self.last_phrases:
            ...
            return
        # Peek last phrase and open correction UI (do not delete yet)
        try:
    ...
    def _open_correction_window(self, phrase_text: str):
        # Create a dedicated UI thread which runs its own Tk mainloop.
        if getattr(self, '_correction_window', None) is not None:
            return

        def ui_thread_func(text_to_display: str):
            try:
                root = tk.Tk()
                root.title('Corregir última frase')
                root.attributes('-topmost', True)
                # Prevent closing via the titlebar
                try:
                ...
                words = [w for w in (text_to_display or '').split()]
                entries = []
                ...
                ok_btn = tk.Button(btn_frame, text='Corregir', command=lambda: self._correction_cmd_queue.put({'cmd': 'apply'}))
                ok_btn.pack(side='left', padx=6)
                cancel_btn = tk.Button(btn_frame, text='Cancelar', command=lambda: self._correction_cmd_queue.put({'cmd': 'cancel'}))
                cancel_btn.pack(side='left', padx=6)

                # Save UI references on the assistant for other methods to query
                ...
                def poll_queue():
                    try:
                        while not self._correction_cmd_queue.empty():
                            item = self._correction_cmd_queue.get_nowait()
                            if not item:
                                continue
                            cmd = item.get('cmd')
                            if cmd == 'cancel':
                                self._close_correction_window_ui()
                                return
                            if cmd == 'apply':
                                # call apply logic in UI thread
                                self._apply_correction_ui()
                                return
                            if cmd == 'delete_word':
                            ...
                            if cmd == 'focus_index':
                                ...
                                if 0 <= idx < len(self._correction_entries):
                                    self._correction_active_index = idx
                                    try:
                                        self._correction_entries[idx].focus_set()
                                    except Exception:
                                        pass
                            if cmd == 'replace_active':
                                val = item.get('value', '')
                                idx = self._correction_active_index
                                if 0 <= idx < len(self._correction_entries):
                                    try:
                                        self._correction_entries[idx].delete(0, 'end')
                                        self._correction_entries[idx].insert(0, val)
                                    except Exception:
                                        pass
                    except Exception:
                        pass
                ...
                print('Error in correction UI thread:', e)
                traceback.print_exc()

        # start UI thread
        t = threading.Thread(target=ui_thread_func, args=(phrase_text,), daemon=True)
        t.start()
    ...
    def _apply_correction_ui(self):
        # Called from the UI thread: read entries and apply correction
        try:
            if not self._correction_entries:
                self._close_correction_window_ui()
                return
            new_words = []
            for e in self._correction_entries:
                try:
                    new_words.append(e.get())
                except Exception:
                    new_words.append('')
            new_phrase = ' '.join([w for w in new_words if w])
            if not self.last_phrases:
            ...
            try:
                ...
                print('Error aplicando corrección (UI):', e)
                traceback.print_exc()
                self.notify('Error al aplicar corrección')
            finally:
                self._close_correction_window_ui()
        except Exception as e:
            print('Error en _apply_correction_ui:', e)
            traceback.print_exc()
            try:
                self._close_correction_window_ui()
            except Exception:
    ...
    def _close_correction_window_ui(self):
        try:
            if self._correction_window:
                try:
                    self._correction_window.destroy()
    ...
    def _handle_correction_command(self, stripped: str, normalized: str) -> bool:
        # If there's no UI active, nothing to do here
        if getattr(self, '_correction_window', None) is None:
            return False
        s = (stripped or '').strip()
        if not s:
            return False
        # enqueue commands for the UI thread to handle
        if re.search(r"\bcancelar\b", s, re.I):
            self._correction_cmd_queue.put({'cmd': 'cancel'})
            self.notify('Corrección cancelada')
            return True
        if re.search(r"\bcorregir\b", s, re.I):
            self._correction_cmd_queue.put({'cmd': 'apply'})
            return True
        if re.search(r"\bborrar palabra\b", s, re.I):
            self._correction_cmd_queue.put({'cmd': 'delete_word'})
            return True
        m = re.search(r"\bpalabra\s+(\d+)\b", s, re.I)
        if m:
            try:
                n = int(m.group(1)) - 1
                self._correction_cmd_queue.put({'cmd': 'focus_index', 'index': n})
                return True
            except Exception:
                pass
        ...
        if m2:
            ...
            if loc:
                self._correction_cmd_queue.put({'cmd': 'replace_active', 'value': loc})
                return True
        return False
